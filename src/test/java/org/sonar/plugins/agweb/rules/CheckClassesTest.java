...
import org.sonar.plugins.agweb.rules.CheckClasses;
...
public class CheckClassesTest {
	...
	public void test() {
		...
		for (Class cls : checks) {
			...
			assertThat(getClass().getResource(testName)).overridingErrorMessage("No test for " + cls.getSimpleName()).isNotNull();
			assertThat(AnnotationUtils.getClassAnnotation(cls, Rule.class) != null).overridingErrorMessage("Add @Rule to " + cls.getSimpleName()).isTrue();
		}
		...
		for (Rule rule : rules) {
			...
			assertThat(getClass().getResource("/org/sonar/l10n/web/rules/Web/" + rule.getKey() + ".html")).overridingErrorMessage(
					"No description for " + rule.getKey()).isNotNull();

			assertThat(rule.getDescription()).overridingErrorMessage("Description of " + rule.getKey() + " should be in separate file").isNullOrEmpty();
			...
			for (RuleParam param : rule.getParams()) {
				...
				assertThat(param.getDescription()).overridingErrorMessage(
						"Description for param " + param.getKey() + " of " + rule.getKey() + " should be in separate file").isNullOrEmpty();
			}
		}
	}
